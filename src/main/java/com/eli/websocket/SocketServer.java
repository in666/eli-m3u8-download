...
public class SocketServer {
    ...
    public void open(Session session, @PathParam(value = "userName") String userName) {
        ...
        socketServers.add(new SocketClient(userName, session));
    ...
    public void onMessage(String message) {
        SocketClient socketClient = socketServers.stream().filter(cli -> cli.getSession() == session).collect(Collectors.toList()).get(0);
        sendMessage(socketClient.getUsername() + "<--" + message, SYS_USERNAME);
        logger.info("客户端:【{}】发送信息:{}", socketClient.getUsername(), message);
    }
    ...
    @OnClose
    public void onClose() {
        socketServers.forEach(socketClient -> {
            if (socketClient.getSession().getId().equals(session.getId())) {
                logger.info("客户端:【{}】断开连接", socketClient.getUsername());
                socketServers.remove(socketClient);
            }
        });
    }
    ...
    public void onError(Throwable error) {
        socketServers.forEach(socketClient -> {
            if (socketClient.getSession().getId().equals(session.getId())) {
                socketServers.remove(socketClient);
                logger.error("客户端:【{}】发生异常", socketClient.getUsername());
    ...
    public synchronized static void sendMessage(String message, String userName) {
        socketServers.forEach(socketClient -> {
            if (userName.equals(socketClient.getUsername())) {
                try {
                    socketClient.getSession().getBasicRemote().sendText(message);
                    logger.info("推送给：【{}】消息：{}", socketClient.getUsername(), message);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
    ...
    public synchronized static int getOnlineNum() {
        return socketServers.stream().filter(socketClient -> !socketClient.getUsername().equals(SYS_USERNAME)).collect(Collectors.toList()).size();
    }
    ...
    public synchronized static List<String> getOnlineUsers() {
        List<String> onlineUsers = socketServers.stream().filter(socketClient -> !socketClient.getUsername().equals(SYS_USERNAME)).map(socketClient -> socketClient.getUsername()).collect(Collectors.toList());
        return onlineUsers;
    }
    ...
    public synchronized static void sendAll(String message) {
        //群发，不能发送给服务端自己
        socketServers.stream().filter(cli -> cli.getUsername() != SYS_USERNAME).forEach(socketClient -> {
            try {
                socketClient.getSession().getBasicRemote().sendText(message);
            } catch (IOException e) {
                e.printStackTrace();
