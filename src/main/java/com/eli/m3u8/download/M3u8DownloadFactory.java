package com.eli.m3u8.download;

import com.eli.m3u8.Exception.M3u8Exception;
import com.eli.m3u8.listener.DownloadListener;
import com.eli.m3u8.utils.Constant;
import com.eli.m3u8.utils.Log;
import com.eli.m3u8.utils.MediaFormat;
import com.eli.m3u8.utils.StringUtils;
...
import static com.eli.m3u8.utils.Constant.FILESEPARATOR;
...
public class M3u8DownloadFactory {
    ...
    public static class M3u8Download {
        ...
        public void start() {
            ...
            if (StringUtils.isEmpty(tsUrl)) Log.i("不需要解密");
            startDownload();
        }
            ...
            new Thread(() -> {
                ...
                while (!fixedThreadPool.isTerminated()) {
                    try {
                        ...
                        for (DownloadListener downloadListener : listenerSet)
                            downloadListener.process(DOWNLOADURL, finishedCount, tsSet.size(), consume, new BigDecimal(finishedCount).divide(new BigDecimal(tsSet.size()), 4, BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(100)).setScale(2, BigDecimal.ROUND_HALF_UP).floatValue(), StringUtils.convertToDownloadSpeed(new BigDecimal(downloadBytes.toString()).subtract(bigDecimal), 3) + "/s");
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                }
            ...
            return new Thread(() -> {
                ...
                while (count <= retryCount) {
                    try {
                        ...
                        if (decrypt == null) outputStream1.write(bytes, 0, available);
...
//                Log.i(urls + "下载完毕！\t已完成" + finishedCount + "个，还剩" + (tsSet.size() - finishedCount) + "个！");
        ...
        private String getTsUrl() {
            ...
            //判断是否是m3u8链接
            if (!content.toString().contains("#EXTM3U")) throw new M3u8Exception(DOWNLOADURL + "不是m3u8链接！");
            ...
            if (StringUtils.isNotEmpty(key1)) key = key1;
            else key = null;
            return key;
        }
        ...
        private String getKey(String url, StringBuilder content) {
            ...
            if (content == null || StringUtils.isEmpty(content.toString())) urlContent = getUrlContent(url, false);
            ...
            while (count <= retryCount) {
                try {
                    ...
                    if (isKey) {
                        ...
                        } else content.append(new String(Arrays.copyOf(bytes, len)));
                        return content;
                    }
    ...
    public static M3u8Download getInstance(String downloadUrl) {
        if (m3u8Download == null) {
            synchronized (M3u8Download.class) {
                if (m3u8Download == null) m3u8Download = new M3u8Download(downloadUrl);
            }
        }
        return m3u8Download;
    }
